...
/* sudo g++ -I ~/Desktop/tmp/VB_cpp/math -I ~/Desktop/tmp/VB_cpp/math/lib/eigen_3.2.9/ -I ~/Desktop/tmp/VB_cpp/math/lib/boost_1.62.0/ -I ~/Desktop/tmp/VB_cpp/math/lib/cvodes_2.9.0/include/ -std=c++11 -stdlib=libc++ VB_reg.cpp -o VB_res_f3.out */
...
struct logh {
  ...
  T operator()(const Matrix<T,Dynamic,1>& parameters) const {
    ...
    T sig2 = parameters(J_);
    T tau2 = parameters(J_+1);
    T gam  = parameters(J_+2);
    ...
    T    w0 = hyperparameters_(4);

    T sigma2;
    Matrix<T,Dynamic,1> mu(J_+3);
    Matrix<T,Dynamic,Dynamic> Sig(J_+3,J_+3);
    // Calculate log-likelihood+log(prior)
    mu = stan::math::multiply(vphi_,theta);
    // Sig = stan::math::multiply(sig2,Diagm_);
    T   res = stan::math::inv_gamma_log(sig2,r_sig/2.,s_sig/2.)+stan::math::inv_gamma_log(tau2,r_tau/2.,s_tau/2.)+stan::math::log(w0)-w0*gam;
    for (int i = 0; i < n_; ++i) {
...
int main() {
  ...
  for (int i = 0; i < n; ++i) {
    fx(i) = 2.*M_PI*x(i)+stan::math::sin(2.*M_PI*x(i));
  }

  double sig2_true = 1.2;
  ...
  Matrix<double,Dynamic,1> s_k(J+3);
  Matrix<double,Dynamic,1> rho(J+3);
  Matrix<double,Dynamic,1> mu_post(J+3);
  Matrix<double,Dynamic,Dynamic> L_post(J+3,J+3);
  for (int i = 0; i < (J+3); ++i) {
    mu_post(i) = 0.;
        s_k(i) = 1.;
        rho(i) = 0.;
  ...
  Matrix<double,Dynamic,Dynamic> thetaContainer(J+3,iterMax);
  Matrix<double,Dynamic,Dynamic> L_1t(J+3,J+3);
  Matrix<double,Dynamic,Dynamic> z_eta(S,J+3);
  Matrix<double,Dynamic,Dynamic> Sig(J+3,J+3);
  Matrix<double,Dynamic,Dynamic> Diag(n,n);
  Matrix<double,Dynamic,1>       theta(J);
  Matrix<double,Dynamic,1>       z(J+3);
  Matrix<double,Dynamic,1>       zeta(J+3);
  Matrix<double,Dynamic,1>       Theta(J+3);
  Matrix<double,Dynamic,1>       grad_Theta;
  Matrix<double,Dynamic,1>       nabla_Tinv(J+3);
  Matrix<double,Dynamic,1>       nabla_logdet(J+3);
  Matrix<double,Dynamic,1>       grad_tmp(J+3);
  nabla_Tinv.fill(1.);
  ...
  while (cont) {
    ...
    Matrix<double,Dynamic,1>       grad_mu(J+3);
    Matrix<double,Dynamic,Dynamic> grad_L(J+3,J+3);
    grad_mu.setZero();
    grad_L.setZero();
    for (int s = 0; s < S; ++s) {
      ...
      sig2 = std::log(std::exp(zeta(J))+1.);
      tau2 = std::log(std::exp(zeta(J+1))+1.);
      gam  = std::log(std::exp(zeta(J+2))+1.);
      for (int i = 0; i < J; ++i) {
        Theta(i) = theta(i);
      }
      Theta(J) = sig2;
  ...
  writeToCSVfile("y_f3",y);
  writeToCSVfile("fx_f3",fx);
  writeToCSVfile("x_f3",x);
  writeToCSVfile("vphi_f3",vphi);
  writeToCSVfile("thetaContainer_f3",thetaContainer);
  writeToCSVfile("L_post_f3",L_post);
