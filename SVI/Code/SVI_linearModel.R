...
for (i in 1:5000) {
  ...
  for (j in 1:S) {
    ...
    f_v         <- logh_v-logq_v
    lb          <- lb+f_v
    gauss_grad  <- gauss_grad+c(beta-muq,vech(tcrossprod(beta)-sigq-tcrossprod(muq)))*f_v
    invgam_grad <- invgam_grad+c(-log(sigma2)+log(Bq)-digamma(Aq),-1/sigma2+Aq/Bq)*f_v
  }
