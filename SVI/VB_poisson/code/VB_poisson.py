...
def logh(parameters,hyperparameters,y,vphi,n,J):
  ...
  vphi_theta = np.dot(vphi,theta)
  res = np.inner(y,vphi_theta)-np.sum(np.exp(vphi_theta))-np.sum(sci.gammaln(y+1.))+logpdfIG(sig2,r_sig/2.,s_sig/2.)+logpdfIG(tau2,r_tau/2.,s_tau/2.)+logpdfIG(psi,a,b)+np.log(w0)-w0*gam
  for j in range(J):
    res = res+logpdfn(theta[j],0,sig2*tau2*np.exp(-(j+1.)*gam))
  return res
...
y = np.empty(n)
for i in range(n):
  y[i] = np.random.poisson(np.exp(fx[i]),size=1)[0]
