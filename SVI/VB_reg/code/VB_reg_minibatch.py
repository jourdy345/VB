...
def logh(parameters,hyperparameters,y,vphi,n,J,B):
  # extract parameters
  theta = parameters[:J]
  psi   = parameters[J]
  sig2  = parameters[J+1]
  ...
  res = logmvnpdf(y,np.dot(vphi,theta),sig2*np.eye(B))/B*n+logpdfIG(sig2,r_sig/2.,s_sig/2.)+logpdfIG(tau2,r_tau/2.,s_tau/2.)+logpdfIG(psi,a,b)+np.log(w0)-w0*gam
  for j in range(J):
    res = res+logpdfn(theta[j],0,sig2*tau2*np.exp(-(j+1.)*gam))
  return res
...
while cont:
  ...
  for s in range(S):
    ...
    # select rows of y and vphi randomly
    row_ind = np.random.randint(0,n,B)
    grad_Theta = grad_logh(Theta,hyperparameters,y[row_ind],vphi[row_ind,:],n,J,B)
  ...
  for s in range(S):
    ...
    gam   = np.log(np.exp(zeta[J+3])+1.)
    Theta = np.hstack([theta,psi,sig2,tau2,gam])
    LB   += logh(Theta,hyperparameters,y,vphi,n,J,n)+np.sum(zeta[(J):])-np.sum(np.log(1.+np.exp(zeta[J:])))-logmvnpdf(zeta,mu_post,Sig)
  LB /= S
