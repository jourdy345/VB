...
def logh(parameters,hyperparameters,y,vphi,n,J):
  ...
  res = np.inner(y,np.dot(vphi,theta))-np.sum(np.log(1.+np.exp(np.dot(vphi,theta))))+logpdfIG(sig2,r_sig/2.,s_sig/2.)+logpdfIG(tau2,r_tau/2.,s_tau/2.)+logpdfIG(psi,a,b)+np.log(w0)-w0*gam
  for j in range(J):
    res = res+logpdfn(theta[j],0,sig2*tau2*np.exp(-(j+1.)*gam))
  return res
