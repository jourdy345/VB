...
\end{enumerate}
\subsection{Lower bound}
As written in the original paper, $\gamma$ should be reparameterized as $\psi = \left|\gamma \right|$ for non-identifiability issue. Regarding the DP,
$$
  \bs{W} = \left\{\bs{V}, \bs{\eta}^{*}, \bs{Z} \right\}
$$
and likewise, regarding the GP,
$$
  \bs{\delta} = \left\{\bs{\beta}, \bs{\theta}_{J}, \sigma^{2}, \tau^{2}, \psi \right\}.
$$
Let $\Theta$ denote all the parameters involved in this model. Then, the lower bound is as follows
\begin{align*}
  \mathcal{L} &= \mathbb{E}\left[\log p \left(\bs{y}|\Theta\right) \right] + \mathbb{E}\left[\log p\left(\bs{V}|\alpha\right)\right] + \mathbb{E}\left[\log p \left(\bs{\eta}^{*}|\lambda\right)\right] \\
  &\quad + \sum_{n=1}^{N}\left(\mathbb{E}\left[\log p \left(Z_{n}|\bs{V}\right)\right]+\mathbb{E}\left[\log p\left(\xi_{n}|Z_{n}\right)\right] \right)\\
  &\quad + \mathbb{E}\left[\log p\left(\bs{\theta}_{J}|\sigma^{2},\tau^{2},\psi\right)\right]+\mathbb{E}\left[\log p\left(\bs{\beta}|\sigma^{2}\right)\right] + \mathbb{E}\left[\log p\left(\tau^{2}\right)\right]\\
  &\quad +\mathbb{E}\left[\log p\left(\sigma^{2}\right)\right]+\mathbb{E}\left[\log p \left(\psi\right)\right] -\mathbb{E}\left[\log q\left(\bs{\delta}\right)\right]-\mathbb{E}\left[\log q\left(\bs{W}\right)\right].
\end{align*}
