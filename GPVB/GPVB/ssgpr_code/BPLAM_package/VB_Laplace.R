appro.gamma = function (a,b) 
  {
    x=a/b
    mean=log(x)
    sec=a
    part1=(2*pi)^(1/2)*abs(sec)^(-1/2)
    ...
    if (a>=100)
      {
        if (a-round(a)<=10^(-2)) 
         {
           part2=c*prod(rep(c,(round(a)-1))/seq(round(a)-1,1,-1))
         } else
          {
            a_f=round(a)
            part2=c^(a-a_f+1)*prod(rep(c,(a_f-1))/seq(a-1,1,-1))/gamma(a-a_f+1)
          }
      }else {part2=c^a/gamma(a)}
    part1 * part2
 }
...
BPLAM_VB<-function (X,Y,l) 
{
    ...
    mu_a <-runif(p);  
    xi_a <-rep(1,p);
    sigma.sq.approx=matrix(NA,ncol=1,nrow=p)
    tau.sq.approx=matrix(NA,ncol=1,nrow=p)
...
#------------------y_stars-------------------#
   ...
   delta0.sq.approx <-appro.gamma(A_delta0.sq,B_delta0.sq) 
   for (i in 1:p)
       {
         sigma.sq.approx[i]=appro.gamma(A_sigma.sq[i],B_sigma.sq[i])
         tau.sq.approx[i]=appro.gamma(A_tau.sq[i],B_tau.sq[i])
       }
...
#-----------------gamma.a---------------------#
   ...
   #------------------------------------#
    ...
    h1.p1 <- log(A_delta0.sq/B_delta0.sq * diag(t(B0)%*%B0)+A_sigma.sq/B_sigma.sq)
    h1.p2 <- (A_delta0.sq/B_delta0.sq)^2 / (A_delta0.sq/B_delta0.sq*diag(t(B0)%*%B0) + 
             A_sigma.sq/B_sigma.sq)  
   ...
   for (i in 1:p) 
    {
      ...
      h1.p1 <- determinant(A_delta0.sq/B_delta0.sq * t(Bj)%*%Bj + A_tau.sq[i]/B_tau.sq[i]* 
                    omega, logarithm = TRUE)$modulus
      h1.p2 <- rep(0,sample_size)
        for (t in 1:sample_size)
         {
           h1.p2[t] <-  A_delta0.sq / B_delta0.sq * Ytemp_i[,t] %*% Bj %*% solve(A_delta0.sq/
                        B_delta0.sq*t(Bj) %*% Bj + A_tau.sq[i]/B_tau.sq[i]*omega) %*% t(Bj) %*% 
                       (A_delta0.sq/B_delta0.sq*Ytemp_i[,t])
         } 
      h1 <- mean(h1.p1-h1.p2)
      ...
      h_gamma.b <- (B_tau.sq[i]/(K*determinant(omega, logarithm = F)$modulus *
                   exp(digamma(A_tau.sq[i]))))^(1/2) * exp(h1 /2)*exp(mean(log(h0))) * 
                    delta0.sq.approx  * tau.sq.approx[i]
