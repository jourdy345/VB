...
\begin{align*}
  \Sigma_{\theta}^{q} &\leftarrow \left(\varphi^{\top}_{J}\varphi_{J} +\frac{r_{q,\sigma}}{s_{q,\sigma}}\frac{r_{q,\tau}}{s_{q,\tau}}\opn{Dg}\left(Q_{1:J}\right) \right)^{-1}\\
...
\begin{align*}
  ...
  s_{q,\sigma} &\leftarrow s_{0,\sigma} +\frac{r_{q,\tau}}{s_{q,\tau}}\sum_{j=1}^{J}\left(\Sigma_{\theta, jj}^{q}+{\mu_{\theta, j}^{q}}^{2}\right)Q_{j} + \Tr\left({\Sigma_{\beta}^{0}}^{-1}\Sigma_{\beta}^{q}\right) +\left(\mu_{\beta}^{q}-\mu_{\beta}^{0}\right)^{\top}{\Sigma_{\beta}^{0}}^{-1}\left(\mu_{\beta}^{q}-\mu_{\beta}^{0}\right)
\end{align*}
\subsubsection{$\beta$}
\begin{align*}
...
\subsection{Model}
  \begin{align*}
    \opn{P}\left(y_{i}=1|\beta, \theta_{J}\right) &= \Phi\left(w_{i}^{\top}\beta + \delta\theta_{J}^{\top}\varphi_{J}^{a}\left(x_{i}\right)\theta_{J}\right)\\
    y_{i}^{*} &= w_{i}^{\top}\beta + \delta \theta_{J}^{\top}\varphi_{J}^{a}\left(x_{i}\right)\theta_{J} + \epsilon_{i}, \quad \epsilon_{i} \sim \mathcal{N}\left(0,1\right)\\
    y_{i} &= \begin{cases}1, & \text{if $y_{i}^{*} \geq 0$}\\ 0, & \text{if $y_{i}^{*} < 0$} \end{cases}\\
...
Joint density:
  \begin{align*}
    ...
    &\quad \times \opn{DE}\left(\psi|0,\omega_{0}\right)\left\{\prod_{i=1}^{n}\left(1\left[y_{i}^{*}\geq 0\right]1\left[y_{i}=1\right] + 1\left[y_{i}^{*}<0\right]1\left[y_{i}=0\right]\right)\phi\left(y_{i}^{*}-w_{i}^{\top}\beta-\delta\theta_{J}^{\top}\varphi_{J}^{a}\left(x_{i}\right)\theta_{J}\right) \right\}
  \end{align*}
\subsection{Update}
\subsubsection{$y_{i}^{*}$}
  \begin{align*}
    \ln q\left(y_{i}^{*}\right) &\propto -\frac{1}{2}\opn{E}_{-y_{i}^{*}}\left({y_{i}^{*}}^{2} -2y_{i}^{*}\left(w_{i}^{\top}\beta +\delta\theta_{J}^{\top}\varphi_{J}^{a}\left(x_{i}\right)\theta_{J}\right)+\left(w_{i}^{\top}\beta + \delta\theta_{J}^{\top}\varphi_{J}^{a}\left(x_{i}\right)\theta_{J}\right)^{2}\right)\\
    &\propto -\frac{1}{2}\left({y_{i}^{*}}^{2} -2y_{i}^{*}\left(w_{i}^{\top}\mu_{\beta}^{q} +\delta\left(\Tr\left(\varphi_{J}^{a}\left(x_{i}\right)\Sigma_{\theta}^{q}\right)+{\mu_{\theta}^{q}}^{\top}\varphi_{J}^{a}\left(x_{i}\right)\mu_{\theta}^{q}\right)\right)\right.\\
    &\quad  +\Tr\left(w_{i}w_{i}^{\top}\Sigma_{\beta}^{q}\right) + {\mu_{\beta}^{q}}^{\top}w_{i}w_{i}^{\top}\mu_{\beta}^{q} + 2\delta w_{i}^{\top}\mu_{\beta}^{q}\left(\Tr\left(\varphi_{J}^{a}\left(x_{i}\right)\Sigma_{\theta}^{q}\right)+ {\mu_{\theta}^{q}}^{\top}\varphi_{J}^{a}\left(x_{i}\right)\mu_{\theta}^{q}\right)\\
    &\quad \left. +2\delta^{2}\Tr\left(\varphi_{J}^{a}\left(x_{i}\right)\Sigma_{\theta}^{q}\right)^{2} + 4\delta^{2}{\mu_{\theta}^{q}}^{\top}\varphi_{J}^{a}\left(x_{i}\right)\Sigma_{\theta}^{q}\varphi_{J}^{a}\left(x_{i}\right)\mu_{\theta}^{q} + \delta^{2}\left(\Tr\left(\varphi_{J}^{a}\left(x_{i}\right)\Sigma_{\theta}^{q}\right)+ {\mu_{\theta}^{q}}^{\top}\varphi_{J}^{a}\left(x_{i}\right)\mu_{\theta}^{q}\right)^{2}\right)\\
    &\propto -\frac{1}{2}\left(y_{i}^{*} - w_{i}^{\top}\mu_{\beta}^{q} - \delta\left(\Tr\left(\varphi_{J}\left(x_{i}\right)\Sigma_{\theta}^{q}\right)+{\mu_{\theta}^{q}}^{\top}\varphi_{J}^{a}\left(x_{i}\right)\mu_{\theta}\right)\right)^{2}
  \end{align*}
Therefore,
  \begin{align*}
...
truncated at zero where
  $$
    \mu_{y_{i}^{*}} = w_{i}^{\top}\mu_{\beta}^{q} + \delta\Tr\left(\varphi_{J}\left(x_{i}\right)\Sigma_{\theta}^{q}\right)+\delta{\mu_{\theta}^{q}}^{\top}\varphi_{J}^{a}\left(x_{i}\right)\mu_{\theta}.
  $$
...
\subsubsection{$\theta$}
  \begin{align*}
    \ln q\left(\theta\right) &\propto \opn{E}_{-\theta}\left[-\frac{1}{2}\sum_{j=1}^{J}\frac{e^{j\left|\psi\right|}\theta_{j}^{2}}{\sigma^{2}\tau^{2}} -\frac{1}{2}\sum_{i=1}^{n}\left(y_{i}^{*}-w_{i}^{\top}\beta -\theta_{J}^{\top}\varphi_{J}^{a}\left(x_{i}\right)\theta_{J}\right)^{2} \right]\\
    &\propto -\frac{1}{2}\opn{E}\left(\frac{1}{\sigma_{2}}\right)\opn{E}\left(\frac{1}{\tau^{2}}\right)\theta_{J}^{\top}\opn{Dg}\left(Q_{1:J}\right)\theta_{J}\\
    &\quad  -\frac{1}{2}\sum_{i=1}^{n}\opn{E}\left(\left(w_{i}^{\top}\beta + \theta_{J}^{\top}\varphi_{J}^{a}\left(x_{i}\right)\theta_{J} \right)^{2} -2y_{i}^{*}\left(w_{i}^{\top}\beta + \theta_{J}^{\top}\varphi_{J}^{a}\left(x_{i}\right)\theta_{J}\right) \right)
  \end{align*}
Seems like the authors of the original paper used NCVMP.
\subsubsection{$\beta$}
  \begin{align*}
    \ln q\left(\beta\right)&\propto \opn{E}_{-\beta}\left(-\frac{1}{2\sigma^{2}}\left(\beta-\mu_{\beta}^{0}\right)^{\top}{\Sigma_{\beta}^{0}}^{-1}\left(\beta-\mu_{\beta}^{0}\right)-\frac{1}{2}\sum_{i=1}^{n}\left(y_{i}^{*}-w_{i}^{\top}\beta -\delta\theta_{J}^{\top}\varphi_{J}^{a}\left(x_{i}\right)\theta_{J}\right)^{2} \right)\\
    &\propto -\frac{1}{2}\left(\beta^{\top}\left(\opn{E}\left(\frac{1}{\sigma^{2}}\right){\Sigma_{\beta}^{0}}^{-1} + W^{\top}W\right)\beta \right.\\
    &\quad  \left. -2\left(\opn{E}\left(\frac{1}{\sigma^{2}}\right)\beta^{\top}{\Sigma_{\beta}^{0}}^{-1}\mu_{\beta}^{0} +\beta^{\top}W^{\top}\mu_{y^{*}}^{q}-\beta^{\top}\delta\sum_{i=1}^{n}w_{i}\left(\Tr\left(\varphi_{J}^{a}\left(x_{i}\right)\Sigma_{\theta}^{q}\right)+{\mu_{\theta}^{q}}^{\top}\varphi_{J}^{a}\left(x_{i}\right)\mu_{\theta}^{q}\right)\right)\right)
  \end{align*}
Therefore, $q\left(\beta\right) = \mathcal{N}\left(\mu_{\beta}^{q}, \Sigma_{\beta}^{q}\right)$ where
\begin{align*}
  \Sigma_{\beta}^{q} &= \left(\opn{E}\left(\frac{1}{\sigma^{2}}\right){\Sigma_{\beta}^{0}}^{-1} + W^{\top}W\right)^{-1}\\
  \mu_{\beta}^{q} &= \Sigma_{\beta}^{q}\left(\opn{E}\left(\frac{1}{\sigma^{2}}\right) {\Sigma_{\beta}^{0}}^{-1}\mu_{\beta}^{0} +W^{\top}\mu_{y^{*}}^{q} -\delta\sum_{i=1}^{n}w_{i}\left(\Tr\left(\varphi_{J}^{a}\left(x_{i}\right)\Sigma_{\theta}^{q}\right) + {\mu_{\theta}^{q}}^{\top}\varphi_{J}^{a}\left(x_{i}\right)\mu_{\theta}^{q}\right)\right)
\end{align*}
\subsubsection{$\tau^{2}$}
  \begin{align*}
    r_{q,\tau} &= r_{0,\tau} + J\\
    s_{q,\tau} &= s_{0,\tau} + \opn{E}\left(\frac{1}{\sigma}\right)\Tr\left(\left(\Sigma_{\theta}^{q*} + \mu_{\theta}^{q*}{\mu_{\theta}^{q*}}^{\top}\right)\opn{Dg}\left(Q_{1:J}\right)\right)
  \end{align*}
\subsubsection{$\sigma^{2}$}
  \begin{align*}
    \ln q\left(\sigma^{2}\right) &\propto -\frac{J}{2}\ln \sigma -\frac{1}{2}
  \end{align*}
\end{document}