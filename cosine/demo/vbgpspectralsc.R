vbgpspectralsc<-function(y,x,Z,T,tol,prior.parms,delta,mupsi.q.start,n.grid) {
  ...
  while(dif>tol | T.old!=T) {
    ...
    if(T>T.old) {
      sigtheta.q.new<-matrix(0,nrow=(T+1),ncol=(T+1))
      sigtheta.q.new[1:(T.old+1),1:(T.old+1)]<-sigtheta.q
      sigtheta.q<-sigtheta.q.new
      mutheta.q.new<-rep(0,times=(T+1))
      mutheta.q.new[1:(T.old+1)]<-mutheta.q
      mutheta.q<-mutheta.q.new
    }
...
#      data.term<-data.term-apply(delta*(temp2-2*wts*delta*term3)*dmats,c(2,3),sum)    
    ...
    sigtheta.q<- -0.5*solve(sigtheta.q.new)
    ...
    mutheta.q<-mutheta.q+sigtheta.q%*%(-e1oversig*diag(e1overupsilon)%*%mutheta.q-0.5*e1oversig2*(apply(term1,2,sum)+data.term))    
    mutheta.q<-as.vector(mutheta.q)
