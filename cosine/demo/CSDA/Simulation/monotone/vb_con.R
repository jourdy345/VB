vbgpspectral_con <-function(y,x,Z,T,tol,prior.parms,delta,mupsi.q.start,mutheta.q.start,n.grid,stepsize,swap,maxit=100) {
...
#  stau.q<-stau.0
  ...
  mubeta.q<-as.matrix(c(mean(y),rep(0,ncol(Z)-1)))
  sigbeta.q<-sigbeta.0
  sigtheta.q<-matrix(0,nrow=(T+2),ncol=(T+2))
...
x.grid <- 1 - x.grid
  ...
  dmatsfull[,1,1]<-(3*x.grid^2-1)/6   #first term
  temparg1<-outer(x.grid,(1:T)) #x*j
  temparg2<-matrix(rep((1:T),each=n.grid),nrow=n.grid,byrow=FALSE)
  dmatsfull[,1,(2:(T+1))]<- -sqrt(2)/((pi*temparg2)^2)*(cos(pi*temparg1))  #Second term
  dmatsfull[,(2:(T+1)),1]<-dmatsfull[,1,(2:(T+1))] #Reflect of second term
  temparg1<-outer((1:T),(1:T),'+')
  temparg2<-outer(rep(1,times=n.grid),temparg1)
  temparg1<-outer(x.grid,temparg1)
  dmatsfull[,(2:(T+1)),(2:(T+1))]<- -cos(pi*temparg1)/(pi*temparg2)^2 #fourth term (Partial)
  temparg1<-outer((1:T),(1:T),'-')
  temparg2<-outer(rep(1,times=n.grid),temparg1)
  temparg1<-outer(x.grid,temparg1)		
  dmatsfull[,(2:(T+1)),(2:(T+1))]<-dmatsfull[,(2:(T+1)),(2:(T+1))] - cos(pi*temparg1)/(pi*temparg2)^2 #fourth term (Partial)
  temparg1<-outer(x.grid,(1:T))
  temparg2<-outer(rep(1,times=n.grid),(1:T))
  temparg3<-outer(x.grid,rep(1,times=T))
  tempmat<- -cos(2*pi*temparg1)/(2*pi*temparg2)^2+(3*temparg3^2-1)/6  #Third Term
  for(j in 2:(T+1)) {
    dmatsfull[,j,j]<-tempmat[,(j-1)]
  }
  dmatsfull2<-array(0,dim=c(n.grid,T+2,T+2))
  dmatsfull2[,(2:(T+2)),(2:(T+2))] <- dmatsfull
if(swap ==1)
{
x.grid <- oldx.grid
dmatsfull2[,1,1] <- (-(x.grid-0.5))
dmatsfull <- dmatsfull2
}
else
{
dmatsfull2[,1,1] <- ((x.grid-0.5))
dmatsfull <- dmatsfull2
  ...
  #  a<-(rsig.0+n+p+(T+1)/2)/2+1 
     a<-(rsig.0+n+p+(T+2)/2)/2+1 
     if(T<T.old) {
      sigtheta.q<-sigtheta.q[1:(T+2),1:(T+2)]
      mutheta.q<-mutheta.q[1:(T+2)]
      if(length(lb)>0) sigma.inv.old<-sigma.inv.old[1:(T+2),1:(T+2)]
      mutheta.q.old<-mutheta.q.old[1:(T+2)]
    }
    if(T>T.old) {
      sigtheta.q.temp<-matrix(0,nrow=(T+2),ncol=(T+2))
      sigtheta.q.temp[1:(T.old+2),1:(T.old+2)]<-sigtheta.q
      sigtheta.q<-sigtheta.q.temp
      mutheta.q.temp<-rep(0,times=(T+2))
      mutheta.q.temp[1:(T.old+2)]<-mutheta.q
      mutheta.q<-mutheta.q.temp
      if(length(lb)>0) {
        sigma.inv.old.temp<-matrix(0,nrow=(T+2),ncol=(T+2))
        sigma.inv.old.temp[1:(T.old+2),1:(T.old+2)]<-sigma.inv.old
        sigma.inv.old<-sigma.inv.old.temp
      }
      mutheta.q.old.temp<-rep(0,times=(T+2))
      mutheta.q.old.temp[1:(T.old+2)]<-mutheta.q.old
      mutheta.q.old<-mutheta.q.old.temp
    }
    dmats<-dmatsfull[,(1:(T+2)),(1:(T+2))]
    ...
    e1overupsilon<-c(1/alpha02,1/sig02,tau.ratio*Qvec)
    ...
    term1<-array(t(apply(dmats,1,matfun1,sigtheta.q)),dim=c(n.grid,T+2,T+2))
    term2<-array(t(apply(dmats,1,matfun2,mutheta.q)),dim=c(n.grid,T+2,T+2))
...
#      data.term<-data.term-apply(delta*(temp2-2*wts*delta*term3)*dmats,c(2,3),sum)    
    ...
    dmatstheta<-array(t(apply(dmats,1,matfun5,mutheta.q)),dim=c(n.grid,T+2))
    term1<-array(t(apply(dmats,1,matfun4,sigtheta.q,mutheta.q)),dim=c(n.grid,T+2))
...
#    e1oversig2<- exp(e1oversig2)*R1*Rv(-b/sqrt(2*cc),(2*a-2))
    ...
    DQvec<-diag(Qvec)
    stau.q<-stau.0+e1oversig*sum(diag((sigtheta.q[3:(T+2),3:(T+2)]+outer(mutheta.q[3:(T+2)],mutheta.q[3:(T+2)]))%*%DQvec))
    tau.ratio<-rtau.q/stau.q
 ...
 #   lbnew<-lbnew-0.5*log(sig02)-T/2*(log(stau.q/2)-digamma(rtau.q/2))    
    lbnew<-lbnew-0.5*log(sig02)-0.5*log(alpha02)-T/2*(log(stau.q/2)-digamma(rtau.q/2))   
...
#    lbnew<-lbnew-(-T/2*log(2*pi)-0.5*as.numeric(determinant(sigtheta.q,logarithm=TRUE)$modulus)-T/2)
    lbnew<-lbnew-(-(T+1)/2*log(2*pi)-0.5*as.numeric(determinant(sigtheta.q,logarithm=TRUE)$modulus)-(T+1)/2)
    ...
    pd2sig<- mfactor*pd1sig-0.5*e1oversig*tau.ratio*sum((diag(sigtheta.q)+mutheta.q^2)*c(0,0,DQsig))
    pd2mu<- mfactor*pd1mu-0.5*e1oversig*tau.ratio*sum((diag(sigtheta.q)+mutheta.q^2)*c(0,0,DQmu))
