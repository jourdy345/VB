...
vbgpspectralsc<-function(y,x,Z,T,tol,prior.parms,delta,mupsi.q.start,mutheta.q.start,n.grid,stepsize,maxit=100) {
...
#  stau.q<-stau.0
  ...
  mubeta.q<-c(mean(y),rep(0,ncol(Z)-1))
  sigbeta.q<-sigbeta.0
...
#    e1oversig2<- exp(e1oversig2)*R1*Rv(-b/sqrt(2*cc),(2*a-2))
 ...
 if(n<=n.grid) {
  ...
  return(list(lb=lb,mutheta.q=mutheta.q,sigtheta.q=sigtheta.q,rtau.q=rtau.q,stau.q=stau.q,sigbeta.q=sigbeta.q,mubeta.q=mubeta.q,sig2psi.q=sig2psi.q,mupsi.q=mupsi.q,dmats=dmats,e1oversig=e1oversig,e1oversig2=e1oversig2,x.grid=x.grid))
