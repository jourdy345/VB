vbgpspectralsc<-function(y,x,Z,T,tol,prior.parms,delta,mupsi.q.start,mutheta.q.start,n.grid,stepsize) {
...
#  stau.q<-stau.0
  ...
  while((dif>tol | T.old!=T) & length(lb)<maxit) {
    ...
    if(T<T.old) {
      ...
      sigma.inv.old<-sigma.inv.old[1:(T+1),1:(T+1)]
